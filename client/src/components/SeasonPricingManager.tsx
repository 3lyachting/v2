    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-1">
          Prix par passager et privatif. Haute saison: juillet-août, du 15/12 au 08/01, et tout février (28/29).
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid grid-cols-[1.3fr_1fr_1fr_1fr_1fr] gap-3 border-b border-slate-100 pb-3 text-xs uppercase tracking-wide text-slate-500">
            <div>Produit</div>
            <div>Haute saison (€ / passager)</div>
            <div>Basse saison (€ / passager)</div>
            <div>Haute saison (€ / privatif)</div>
            <div>Basse saison (€ / privatif)</div>
          </div>

          <div className="space-y-3 pt-3">
            {productKeys.map((product) => (
              <div key={product} className="grid grid-cols-[1.3fr_1fr_1fr_1fr_1fr] gap-3 items-center">
                ...
                <input
                  type="number"
                  min={0}
                  value={pricing[product]?.highSeasonPrivate ?? ""}
                  onChange={(e) => updatePrice(product, "highSeasonPrivate", e.target.value)}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
                ...
                <input
                  type="number"
                  min={0}
                  value={pricing[product]?.lowSeasonPrivate ?? ""}
                  onChange={(e) => updatePrice(product, "lowSeasonPrivate", e.target.value)}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
